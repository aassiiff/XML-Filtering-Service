 ...
 */
@WebFilter(filterName = "/SessionFilter", urlPatterns = { "/redirect" })
public class SessionFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		/*
		 * If request has jsessionID; then retrieve it from saved sessions. If
		 * session matching jsessionID is found then process it Else If session
		 * matching jsessionID is not found then ignore jsessionID and create
		 * new Session
		 */
		if (sessionSaved != null) {
			sessionNew = ((HttpServletRequest) request).getSession(false);
			/*
			...
			/*
			 * Test if saved session and new session have same ID If they have
			 * same IDś then nothing to do Else copy session attributes from the
			 * saved session to new Session.
			 */
			if (sessionNew.getId().equalsIgnoreCase(sessionSaved.getId())) {
				...
				ArrayList<UploadedXSAMS> dataHolder = (ArrayList<UploadedXSAMS>) sessionSaved
						.getAttribute("uploadedFiles");
				if (dataHolder != null) {
					sessionNew.setAttribute("uploadedFiles", dataHolder);
					System.out
							.println("Session Found and Both Sessions are not same and UploadedFiles is NOT null in old session: "
									+ dataHolder.size());
				} else {
					System.out
							.println("Session Found and Both Sessions are not same and UploadedFiles is null in old session");
				}
			}
		} else {
			System.out.println("No Session Mentioned in the URL");
			sessionNew = ((HttpServletRequest) request).getSession(true);
		}
		 ...
		 * (attrs.hasMoreElements()) { System.out.println(attrs.nextElement());
		 * }
		 */
	...
	private HttpSession getSession(HttpServletRequest request) {
		...
		if (sessionID == null) {
			sessionID = request.getParameter("sessionRef");
		...
		if (sessionID != null) {
			request.setAttribute("oldSessionID", sessionID);
	...
	private String checkAndGetSession(HttpServletRequest request) {
		...
		if (index > 1) {
			...
			System.out
					.println("checkAndGetSession(HttpServletRequest request): "
							+ sessionID);
		} else {
			String queryString = request.getQueryString();
			if (queryString != null && queryString.length() > 30) {

				int indexOfParameter = queryString.indexOf("sessionRef=");

				// 11 is the length of sessionRef=
				int index2 = queryString.indexOf("?", indexOfParameter + 11);
				if (index2 > 0) {
					sessionID = queryString.substring(indexOfParameter + 11, index2);
				} else {
					sessionID = queryString.substring(indexOfParameter + 11);
				}
				System.out
						.println("Manual checkAndGetSession(HttpServletRequest request): "
								+ sessionID);
			}
		}
